...
import { categories, coffeeItems } from '../config';
...
export default function HomeScreen() {
  ...
  const [coffeeFilterItem, setCoffeeFilterItem] = useState(coffeeItems);
  const [searchString, setSearchString] = useState(null);

  const filterCategories = (id) => {
    let filterData = coffeeItems
    setActiveCategory(id)
    if(id != null){
        filterData = coffeeItems.filter(item => item.categoryId === id);
      }
    setCoffeeFilterItem(filterData)
  }

  const searchFilter = () => {
    let filterData = coffeeItems
    if(searchString != null || searchString.trim() != ""){
      filterData =  coffeeItems.filter(item => item.name.includes(searchString));
      ...
      <SafeAreaView className={ios? '-mb-8': ''}>
        ...
        <View className="mx-5 shadow" style={{marginTop: height*0.06}}>
          <View className="flex-row items-center rounded-full p-1 bg-[#e6e6e6]">
            <TextInput onChangeText={text => setSearchString(text)} value={searchString} placeholder='Search' className="p-4 flex-1 font-semibold text-gray-700" />
            <TouchableOpacity 
